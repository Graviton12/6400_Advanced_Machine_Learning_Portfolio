...
* The MSE is the average of the squared differences between actual values and predicted values. In context to Efron's paper, it is the squared difference between the true error rate and the estimated error rate. The formula used for the MSE for comparing both error rates is given by:
$$
MSE = E(\widehat{Err} - Err)^2
$$
Where: 
$$
\widehat{Err} = the\ estimated\ error\ rate
$$
$$
Err = the\ true\ error\ rate
$$
...
![Figure 3: Visual for the Linear Discriminant.](images/linear-discriminant.png)
...
3. Efron has the apparant error rate which is used as a baseline for comparison.
...
6. He then repeats the experiment x number of times.
...
![Figure 4: Diagram showing the design for Efron's simulation study.](images/efron-1983-simulation-study-design.png)
...
![Figure 5: Diagram showing the ordrinary bootstrapping process for the simulation study.](images/ordinary-bootstrap-replicaiton-per-trial.png)
...
![Figure 6: Diagram showing the design for the simulation study I am choosing to replicate. The following diagram is using the (2.14) sampling experiment.](images/simulation-study-replication-plan.png)

## Simulation Study of Efron's Paper
### Coding presented below: